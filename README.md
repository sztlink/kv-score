...
**Trajectory fidelity is not decision goodness.** A higher LCP means the whole generation reproduces fp16's exact tokens longer; it does not mean the actual task decision is more robust. I checked this with @spiritbuun's `margin_bench` (logprob distance-to-flip on the routing answer, paired per-case t-stats) pointed at the same vLLM configs. At 14B, depth 8192, where exact-match is fully saturated (120/120 for every config), the margin still discriminates, but among the accurate configs the decision robustness is comparable: KVarN k4v2 vs TurboQuant k8v4 is t = -0.03 (no difference), and only the aggressive 2-bit-V preset (tq-k4v2nc) degrades clearly (worst-case margin 0.12, nearly a flip). So KVarN reproduces the fp16 trajectory far longer, and on the routing decision itself KVarN and TurboQuant are about even. Data: `results/margin-bench-buun-2026-06-13.md`.

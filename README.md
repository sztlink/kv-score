...
- **Not scored, integration bugs (not method verdicts)**: AdaKV under SDPA degenerates (needs flash-attention). transformers `QuantizedCache` (both quanto and hqq backends, int4/int2) produces corrupted generations on this rig (transformers 4.x + torch 2.12/cu130) while the fp16 control in the identical harness is exact; int4 KV is near-lossless in vLLM on the same model, so this is an upstream integration bug, tracked for a minimal repro report.

...
- **Not scored, integration bugs (not method verdicts)**: AdaKV under SDPA degenerates (needs flash-attention). transformers `QuantizedCache` (both quanto and hqq backends) produced corrupted generations under our calling convention on this rig, while the fp16 control in the identical harness is exact. Cause not yet attributed (could be our config, a version incompatibility on a bleeding-edge stack, or upstream); not scored either way until that is settled.
